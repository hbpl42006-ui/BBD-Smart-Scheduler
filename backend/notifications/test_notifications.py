...
def test_meta_provider_request_and_provider_id(users,settings):
    first,_=users;first.whatsapp_number='+919876543210';first.save(update_fields=['whatsapp_number']);pref=NotificationPreference.objects.get(user=first);pref.email_enabled=False;pref.whatsapp_enabled=True;pref.whatsapp_opt_in=True;pref.save();settings.WHATSAPP_ENABLED=True;settings.WHATSAPP_ACCESS_TOKEN='secret-token';settings.WHATSAPP_PHONE_NUMBER_ID='phone-id';settings.WHATSAPP_TEMPLATE_TIMETABLE_UPDATED='timetable_updated';settings.WHATSAPP_DEFAULT_LANGUAGE='en'
