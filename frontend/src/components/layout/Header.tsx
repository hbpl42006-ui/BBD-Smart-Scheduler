...
import { NOTIFICATION_UNREAD_CHANGED, unreadNotificationCount } from '@/lib/api/notifications';
...
export function Header({ title }: { title: string }) {
  ...
  useEffect(() => {
    ...
    unreadNotificationCount().then(setUnread).catch(() => undefined);
    const onUnreadChanged = (event: Event) => {
      const count = (event as CustomEvent<{ count?: number }>).detail?.count;
      if (typeof count === 'number') setUnread(count);
    };
    window.addEventListener(NOTIFICATION_UNREAD_CHANGED, onUnreadChanged);
    return () => window.removeEventListener(NOTIFICATION_UNREAD_CHANGED, onUnreadChanged);
  }, []);
  ...
  return (
    <header className="no-print sticky top-0 z-10 flex h-16 items-center gap-4 border-b bg-white/95 px-4 sm:px-8">
      ...
      <Link href="/notifications" className="relative rounded-md p-2 text-slate-500 hover:bg-slate-100" aria-label="Notifications"><Bell className="h-5 w-5" />{unread>0&&<span className="absolute -right-1 -top-1 min-w-4 rounded-full bg-blue-600 px-1 text-center text-[10px] leading-4 text-white">{unread>99?'99+':unread}</span>}</Link>
